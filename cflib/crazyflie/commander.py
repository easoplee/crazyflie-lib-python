...
class Commander():
    ...
    def send_manual_setpoint(self, 
                             rollrate: float, # in degrees/s
                             pitchrate: float, # in degrees/s
                             yawrate: float, # in degrees/s
                             thrust):
        ...
        rate = int(True)
        pk.data = struct.pack('<BfffHB', TYPE_MANUAL, 
                              rollrate, pitchrate, yawrate, thrust, rate)
        self._cf.send_packet(pk)
